...
import { blockAtTime, bookingAtTime, bookingWindow, goToDayOffset, holdAtTime, sessionById, statusAtTime } from './domain.js';
...
async function saveAdvance(){
  ...
  const result=await mutate(`/api/bookings/${encodeURIComponent(session.id)}/payments`,{
    amount,mode:S.advPayMode,settle:S.advMode==='settle',reason:S.advReason});
  if(result){S.advAsk=null;S.advVal='';S.advReason='';}
}
...
async function confirmAuditedAction(){
  ...
  let result=null;
  if(ask.kind==='booking')result=await mutate(`/api/bookings/${encodeURIComponent(ask.id)}/release`,{reason});
  ...
  if(ask.kind==='status')result=await mutate(`/api/bookings/${encodeURIComponent(ask.id)}/status`,{status:ask.target,reason,atMinute:nowMin()});
  ...
  switch(action){
    ...
    case 'sel-primary':{const start=selectedStart(),status=statusAtTime(S.sel.di,S.pitch,start),hour=hours()[S.sel.hi];
      if(status==='free'){if(!bookingWindow(S.sel.di,S.pitch,start,S.dur).ok)return;S.confirm='book';S.formOpen=false;S.form=blankForm();}
        ...
        title:'Reopen maintenance slot',subject:`${PITCHES[S.pitch].name} · ${hourRng12(hour)}`};S.actionReason='';}}break;}
    ...
    case 'plus30':await mutate(`/api/bookings/${encodeURIComponent(id)}/extend`,{minutes:30});break;
    ...
    case 'timer-start':{const current=sessionById(S.timerAsk);const result=await mutate(`/api/bookings/${encodeURIComponent(current.id)}/status`,
      {status:'running',atMinute:value==='now'?nowMin():current.start});if(result)S.timerAsk=null;break;}
    ...
    case 'done-confirm':{const current=sessionById(S.doneAsk);const result=await mutate(`/api/bookings/${encodeURIComponent(current.id)}/status`,
      {status:'done',atMinute:nowMin()});if(result)S.doneAsk=null;break;}
    case 'done-collect':{const current=sessionById(S.doneAsk);const result=await mutate(`/api/bookings/${encodeURIComponent(current.id)}/status`,
      {status:'done',atMinute:nowMin()});if(result){S.doneAsk=null;pickPay(sessionById(current.id),'Payment done');}break;}
    case 'adv-cancel':S.advAsk=null;S.advVal='';S.advReason='';break;
    case 'adv-quick':S.advVal=value;break;
    case 'adv-mode':S.advPayMode=value;break;
...
async function handleSubmit(event){
  ...
  try{const auth=await apiRequest('/api/auth/login',{method:'POST',body:credentials});S.user=auth.user;S.csrf=auth.csrfToken;await loadState();}
...
function refreshGate(){
  ...
  const advance=document.querySelector('[data-act="adv-save"]');if(advance&&S.advAsk){const session=sessionById(S.advAsk),value=Math.max(0,parseInt(S.advVal,10)||0);
    ...
    advance.disabled=!(value>0&&value<=outstanding&&(!discounting||(can('manager')&&S.advReason.trim().length>=5)));}
...
function handleKeydown(event){
  ...
  if(S.confirm)S.confirm=null;else if(S.actionAsk){S.actionAsk=null;S.actionReason='';}
  else if(S.advAsk!=null){S.advAsk=null;S.advVal='';}else if(S.doneAsk!=null)S.doneAsk=null;else if(S.timerAsk!=null)S.timerAsk=null;
